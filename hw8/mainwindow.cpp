    ...
    , ui(new Ui::MainWindow)
{
    ...
    */
    ...
    /*
     * Соединяем сигналы, которые передает ответ от БД с методами, которые отображает ответ в ПИ
     */
...
void MainWindow::on_pb_request_clicked()
{
    ...
    if(category == "Комедия")
    {
// В базе данных фильмов не отражены категории (жанры) фильмов
...
// Вывод для примера фильма при выборе жанра "комедия"
        request = "SELECT title, description FROM film WHERE film_id = 11";
    }
...
// Вывод для примера фильма при выборе жанра "ужасы"
        request = "SELECT title, description FROM film WHERE film_id = 33";
    }
    else
    {
        request = "*";
